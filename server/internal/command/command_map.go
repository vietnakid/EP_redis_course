...
func handleSet(store *datastructure.Store, args []string) []byte {
	if len(args) < 2 {
	...
	store.ReplaceKind(key, datastructure.KindString)
	store.SetString(key, e)
	return protocol.EncodeSimpleString("OK")
}

func handleGet(store *datastructure.Store, args []string) []byte {
	if len(args) != 1 {
		return protocol.EncodeError("ERR wrong number of arguments for 'GET'")
	}
	if reply := checkType(store, args[0], datastructure.KindString); reply != nil {
		return reply
	}
	e, ok := store.GetLiveString(args[0])
	if !ok {
...
func handleTTL(store *datastructure.Store, args []string, unit time.Duration) []byte {
	if len(args) != 1 {
...
func handleExpire(store *datastructure.Store, args []string) []byte {
	if len(args) != 2 {
...
func handleDel(store *datastructure.Store, args []string) []byte {
	if len(args) < 1 {
...
func handleExists(store *datastructure.Store, args []string) []byte {
	if len(args) < 1 {
