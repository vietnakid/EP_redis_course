...
// SMEMBERS. Each key routes through store.SetStore, created lazily on
// first SADD.
package command
...
func handleSAdd(store *datastructure.Store, args []string) []byte {
	if len(args) < 2 {
		return protocol.EncodeError("ERR wrong number of arguments for 'sadd' command")
	}
	key := args[0]
	if reply := checkType(store, key, datastructure.KindSet); reply != nil {
		return reply
	}
	set, exists := store.SetStore[key]
	if !exists {
		set = datastructure.NewSimpleSet()
		store.SetStore[key] = set
	}
...
func handleSRem(store *datastructure.Store, args []string) []byte {
	if len(args) < 2 {
...
func handleSIsMember(store *datastructure.Store, args []string) []byte {
	if len(args) != 2 {
...
func handleSMembers(store *datastructure.Store, args []string) []byte {
	if len(args) != 1 {
