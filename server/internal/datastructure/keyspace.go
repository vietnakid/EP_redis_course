...
// be: a plain string, a SimpleSet, or a SortedSet. Each lives in its own
// package-level map so the command package's routing layer doesn't need to
// touch storage internals - it looks a key up by kind, gets back the
// structure, and calls methods on it.
package datastructure

// Kind identifies which of the three keyspaces a key currently lives in. A
